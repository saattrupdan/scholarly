...
def fetch(category="", max_results=5, start=0):
    ...
    INPUT
        ...
        max_results=5:  int, maximal number of papers scraped
                        (arXiv limits this to 30,000)
        start=0:        int, from which paper the scraping begins

    OUTPUT
        pandas DataFrame object with ids, titles, authors, abstracts,
        categories, pdf links, updated date and published date.
    """
    ...
    papers = {'id' : [], 'title' : [], 'authors' : [], 'abstract' : [],
              'category' : [], 'pdf' : [], 'updated' : [], 'published' : []}
    for entry in soup.find_all(name="entry"):
        # save metadata
        try:
            entry_id = entry.id.string
            entry_updated = dt.strptime(entry.updated.string[:10], "%Y-%m-%d")
            entry_published = dt.strptime(entry.published.string[:10],
                                "%Y-%m-%d")
...
def batch_fetch(category = "", max_results = 5, file_path = "paper_data",
    ...
    INPUT
        category="":                str, arXiv category
        max_results=5:              int, maximal number of papers scraped
        file_path="paper_data":     str, csv file path, can include file 
                                    extension or not
        start=0:                    int, from which paper the scraping begins
        batch_size=1000:            int, how many papers are loaded at a time 
                                    (arXiv limits this to 30,000)
    ...
    for i in range(batches):
        ...
        # make a loop to keep trying to get data in case arxiv
        # spits out nothing for some reason 
    ...
    # merge all .csv files
    df = pd.concat( [ pd.read_csv(f"{file_path}_{i}.csv") 
            for i in range(batches) ] )
...
def cat_scrape(max_results_per_cat=10000, file_path="arxiv_data",
    ...
    # merge all .csv files
    df = pd.concat( [ pd.read_csv(f"{file_path}_{cat}.csv") 
            for cat in all_cats ] )
    df.to_csv(f"{file_path}.csv", index=False)
