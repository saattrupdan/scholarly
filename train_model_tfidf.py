...
def train_model(file_names, info, data_path = 'data', POS_WEIGHT = 5,
    ...
    for file_name in file_names:
        ...
        # load tf-idf model 
        full_path = os.path.join(data_path, f'{file_name}_tfidf_model.pickle')
        with open(full_path, 'rb+') as pickle_in:
            tfidf = pickle.load(pickle_in)

        # load validation set
        full_path = os.path.join(data_path,
            f"{info['val']}_{info['1hot']}.csv")
        X_val = np.asarray(pd.read_csv(full_path))[:, 0]
        X_val = tfidf.transform(X_val).todense()
        Y_val = np.asarray(pd.read_csv(full_path))[:, 1:].astype(int)
        ...
        if os.path.isfile(full_path) and save_model:
            ...
            # get data
            full_path = os.path.join(data_path, f"{file_name}_tfidf.npz")
            X_train = load_npz(full_path)
            full_path = os.path.join(data_path,
                f"{file_name}_{info['1hot']}.csv")
            Y_train = np.asarray(pd.read_csv(full_path, header = None))
